...
@CrossOrigin(origins = "*")
public class SizesDtoController{
	
	SizesDtoService sizesDtoService;
	
	public SizesDtoController(SizesDtoService sizesDtoService) {
		this.sizesDtoService = sizesDtoService;
	}
	
	// Método para crear una nueva talla
		@PostMapping
		public ResponseEntity<SizesDto> createSizes(@RequestBody Sizes sizes){
			SizesDto newSizes = sizesDtoService.createSizes(sizes);
			return ResponseEntity.status(201).body(newSizes);
		}
		
		// Método para obtener una talla por ID
		@GetMapping("{id}") // api/v1/addresses/10
		public ResponseEntity<SizesDto> getSizesById(@PathVariable("id")Long id){
			SizesDto existingSizes = sizesDtoService.getSizesById(id);
			return ResponseEntity.ok(existingSizes);
		}
		
		// Método para actualizar una talla
	    @PutMapping("{id}")
	    public ResponseEntity<SizesDto> updateSizes(@RequestBody Sizes newSizesData, @PathVariable("id") Long id) {
	    	SizesDto updatedSizes = sizesDtoService.updateSizes(newSizesData, id);
	    ...
	    public ResponseEntity<String> deleteSizes(@PathVariable("id") Long id) {
	    	sizesDtoService.deleteSizes(id);
